...
import { addBorrowBookForUser } from "services/userService";
...
const RegisterBorrowBook = ({ fetchBorrowedBooks }) => {
  ...
  const fetchBooks = async (
    ...
    isAvailable = selectedAvailability // Add availability filter
  ) => {
    setLoading(true);
    try {
      const response = await getAllBooks(
        ...
        authorId,
        publisherId,
        isAvailable // Pass availability to the API
      );
      setBooks(response.data); // Assuming response has 'books' array
      setTotalBooks(response.meta.total); // Assuming response has 'total' books count
    } catch (error) {
